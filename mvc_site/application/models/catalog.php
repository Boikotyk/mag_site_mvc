...
//Модель вывода каталога
 class Application_Models_Catalog extends Lib_DateBase
  {	  
	  ...
	  { 
		 if(!$this->getCurrentCategory()){ echo "Помилка отриманих даних!"; exit; }//Если неудалось получить текущую категорию
		...
		if($this->current_category["url"]=="mvc_site/catalog"){
	...
	//	if(empty($this->category_id)) $sql = "SELECT  id  FROM product";//если категория не выбранна, то показать все товары каталога
	  ...
	  { 
	    if(!$this->getCurrentCategory()){ echo "Помилка отримання даних!"; exit; }//Если неудалось получить текущую категорию
	...
	//	if(empty($this->category_id)) $sql = "SELECT  id  FROM product";//если категория не выбранна, то показать все товары каталога
		...
		if($count>1){
			for($page=0; $page<$count; $page++){// перебираем все страницы и формируем ссылки на них
			 ...
			 $pages='<div class="pagination">Сторінка '.($activ_page+1).' из '.($count).' '.$pages.'</div>';
		}
		// дописывает  к возвращаемому массиву информацию о пагинации  		  
		$сatalogItems['pagination']=$pages;
	   ...
	   function getCurrentCategory(){
			...
			$this->current_category['url']="mvc_site/catalog";
