  ...
  {	  
		...
		function isValidData($array_data){
			...
			if(!preg_match("/^[A-Za-z0-9._-]+@[A-Za-z0-9_-]+.([A-Za-z0-9_-][A-Za-z0-9_]+)$/", $array_data['email'])){ 
			  $error="E-mail не існує!";	
			} 
			// заполненность адреса
			else if(!trim($array_data['adres'])){ 
			  $error="Введіть адресу!";	
			}
	...
	//добавление заявки		
	function addOrder(){
		$date = date('y-m-d h:m:s'); //текущая дата в UNIX формате
...
$connects = mysqli_connect(HOST, USER, PASSWORD, 'pros');
		foreach ($array as $key => $value) {
			$keyT[] = $key;
			$valueT[] = '"'.mysqli_real_escape_string($connects, $value).'"';
		}

$queryT = "INSERT INTO `order` (".implode(',', $keyT).') VALUES ('.implode(',', $valueT).")";

		parent::build_query($queryT);
